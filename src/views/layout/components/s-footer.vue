<template>
    <div class="s-footer">
        <div class="s-footer-container clearfix">
            <div class="fl fl-left">
                <p>湖南省健康人才网</p>
                <p class="s">HUNAN HEALTH TALENT SERVICE WEBSITE</p>
            </div>
            <div class="fl fl-right">
                <p class="s">
                    <span>地址：长沙市蔡锷北路529号宏建大厦二楼湖南省健康服务业协会(湘雅医院老门诊楼斜对面长沙电子工业学校旁）</span>
                    <span>邮编：410005</span>
                </p>
                <p class="s">
                    <span>联系方式：付老师   18229734107  </span>
                    <span>座机：0731-84831417</span>
                </p>
                <p class="s">
                    <span>版权所有：© 2018-2050    湖南盛世锦康医疗管理咨询有限公司    湘ICP备18018866号</span>
                </p>
            </div>
        </div>
    </div>
</template>
<script>

</script>
<style rel="stylesheet/scss" lang="scss" scoped>
    .s-footer {
        height: 126px;
        background-color: #e6e6e6;
        border-top: 1px solid #dddddd;
        .s-footer-container {
            width: 1000px;
            margin: 18px auto 14px;
            height: 94px;
            color: #727272;
            .fl-left {
                padding-top: 20px;
                width: 40%;
                background-image: url("../../../assets/footer/line.png");
                background-repeat: no-repeat;
                background-position: right center;
            }
            .fl-right {
                width: 60%;
                padding: 15px 10px 0 80px;
            }
            .fl {

                p {
                    font-size: 30px;
                    margin-bottom: 5px;
                }
                .s {
                    font-size: 12px;
                }
            }
        }
    }
</style>
